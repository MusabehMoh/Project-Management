...
export default function TeamKanbanBoard({ onTaskUpdate }: TeamKanbanBoardProps) {
  ...
  const initializeColumns = (tasks: MemberTask[]): KanbanColumn[] => {
    ...
    const relevantStatuses = taskStatuses.filter(status => status.value >= 1 && status.value <= 5);
                        ...
                        <CardBody className="p-3 space-y-2">
                          ...
                          <div className="space-y-1 text-xs text-default-500">
                            {task.project && (
                              <div className="line-clamp-1">
                                <strong>{t("teamDashboard.kanban.project")}:</strong>{" "}
                                {task.project.applicationName}
                              </div>
                            )}
                            {task.requirement && (
                              <div className="line-clamp-1">
                                <strong>{t("teamDashboard.kanban.requirement")}:</strong>{" "}
                          ...
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className={task.progress === 100 ? "text-success" : "text-default-600"}>
                                {task.progress}%
                              </span>
                            </div>
                          </div>
