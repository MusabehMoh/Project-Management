...
import { Calendar, Clock, Layers, TrendingUp, ChevronDown, User, X, Search, SlidersHorizontal } from "lucide-react";
...
export default function AllProjectsOverview({ projects }: AllProjectsOverviewProps) {
  ...
  useEffect(() => {
    ...
    if (restoreParam) {
      ...
      if (projectData) {
        ...
        if (timelineId) {
          const timeline = projectData.timelines.find(t => t.id === timelineId);
          if (timeline) {
  ...
  const filteredAndSortedProjects = useMemo(() => {
    ...
    if (progressFilter !== "all") {
      filtered = filtered.filter(({ stats }) => {
        ...
        if (progressFilter === "medium") return stats.avgProgress >= 40 && stats.avgProgress < 70;
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case "name":
          return a.project.applicationName.localeCompare(b.project.applicationName, language);
        case "progress":
      ...
      <Card>
        <CardBody className="text-center py-12">
          <div className="space-y-4">
            ...
            <div>
              <p className="text-lg text-default-600">{t("timeline.noProjectsAvailable")}</p>
              <p className="text-sm text-default-500">{t("timeline.noTimelinesGeneral")}</p>
            </div>
          </div>
        </CardBody>
      </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardBody>
          <div className={`grid grid-cols-1 lg:grid-cols-3 gap-4 ${isRTL ? "text-right" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
            {/* Search */}
            <Input
            ...
            >
              ...
              <SelectItem key="progress">{t("timeline.sortByProgress")}</SelectItem>
              <SelectItem key="timelines">{t("timeline.sortByTimelines")}</SelectItem>
              <SelectItem key="sprints">{t("timeline.sortBySprints")}</SelectItem>
            ...
            >
              ...
              <SelectItem key="medium">{t("timeline.mediumProgress")}</SelectItem>
          ...
          {(searchQuery || progressFilter !== "all") && (
            <div className={`flex items-center gap-2 mt-4 flex-wrap ${isRTL ? "" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
              <span className="text-sm text-default-500">{t("timeline.activeFilters")}:</span>
              {searchQuery && (
                <Chip
                  size="sm"
                  variant="flat"
              ...
              {progressFilter !== "all" && (
                ...
                >
                  {t("common.progress")}: {progressFilter === "low" ? t("timeline.lowProgress") : progressFilter === "medium" ? t("timeline.mediumProgress") : t("timeline.highProgress")}
                </Chip>
              )}
              <Button
                size="sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardBody className="flex flex-row items-center gap-3 py-4" dir={isRTL ? "rtl" : "ltr"}>
            <div className="p-3 rounded-lg bg-primary/10">
              <BuildingIcon className="w-6 h-6 text-primary" />
            </div>
            <div className={isRTL ? "text-right" : ""}>
              <p className="text-sm text-default-500">{t("common.projects")}</p>
              <p className="text-2xl font-semibold">{filteredAndSortedProjects.length}</p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody className="flex flex-row items-center gap-3 py-4" dir={isRTL ? "rtl" : "ltr"}>
            ...
            <div className={isRTL ? "text-right" : ""}>
              <p className="text-sm text-default-500">{t("timeline.timelines")}</p>
              <p className="text-2xl font-semibold">
                {filteredAndSortedProjects.reduce((acc, p) => acc + p.timelines.length, 0)}
              </p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody className="flex flex-row items-center gap-3 py-4" dir={isRTL ? "rtl" : "ltr"}>
            ...
            <div className={isRTL ? "text-right" : ""}>
              <p className="text-sm text-default-500">{t("timeline.sprints")}</p>
              <p className="text-2xl font-semibold">
                {filteredAndSortedProjects.reduce((acc, p) => acc + p.stats.totalSprints, 0)}
              </p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody className="flex flex-row items-center gap-3 py-4" dir={isRTL ? "rtl" : "ltr"}>
            ...
            <div className={isRTL ? "text-right" : ""}>
              ...
              <p className="text-2xl font-semibold">
                {filteredAndSortedProjects.reduce((acc, p) => acc + p.stats.totalTasks, 0)}
              </p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody className="text-center py-12">
            <div className="space-y-4">
              ...
              <div>
                <p className="text-lg text-default-600">{t("timeline.noResultsFound")}</p>
                <p className="text-sm text-default-500">{t("timeline.tryDifferentFilters")}</p>
              </div>
              <Button
                variant="flat"
                onPress={() => {
            ...
            >
              <Accordion
                ...
                selectedKeys={selectedProjectId ? [selectedProjectId.toString()] : []}
                onSelectionChange={(keys) => {
                  const keysArray = Array.from(keys);
                  const selected = keysArray.length > 0 ? parseInt(keysArray[0] as string) : null;
                ...
                {filteredAndSortedProjects.map(({ project, timelines, stats }) => (
                    ...
                    title={
                      ...
                      >
                        ...
                        <div className={`flex-1 ${isRTL ? "text-right" : ""}`}>
                          <h3 className="text-base font-semibold">{project.applicationName}</h3>
                          <div
                          ...
                          >
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-default-600">
                                {timelines.length} {timelines.length === 1 ? t("timeline.timeline") : t("timeline.timelines")}
                              </span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <Layers className="w-4 h-4 text-default-400" />
                              <span className="text-default-600">{stats.totalSprints} {t("timeline.sprints")}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <Clock className="w-4 h-4 text-default-400" />
                              <span className="text-default-600">{stats.totalTasks} {t("timeline.tasks")}</span>
                            </div>
                          </div>
                        </div>
                        {stats.totalTasks > 0 && (
                  ...
                  >
                    <div className="pb-4 space-y-3" dir={isRTL ? "rtl" : "ltr"}>
                      ...
                      {stats.totalTasks > 0 && (
                        <div className="space-y-1">
                          <div className={`flex items-center text-sm justify-between`}>
                            {isRTL ? (
                              <>
                                <span className="text-default-500">{t("common.progress")}</span>
                                <span className="font-semibold text-default-700">{stats.avgProgress}%</span>
                              </>
                            ) : (
                              <>
                                <span className="text-default-500">{t("common.progress")}</span>
                      ...
                      {timelines.length > 0 ? (
                        <div className="space-y-2">
                          <p className={`text-xs text-default-500 font-medium uppercase ${isRTL ? "text-right" : ""}`}>
                            ...
                            {timelines.map((timeline) => (
                              ...
                              >
                                <div className="space-y-1.5">
                                  ...
                                  <div className={`flex items-center gap-1.5 text-[10px] text-default-500 ${isRTL ? "flex-row-reverse justify-end" : ""}`}>
                                    ...
                                    <div className={`flex items-center gap-0.5 ${isRTL ? "flex-row-reverse" : ""}`}>
                                      ...
                                      <span>{(timeline.sprints || []).reduce((acc, s) => acc + (s.tasks?.length || 0), 0)}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className="text-center py-4">
                          <p className="text-sm text-default-400">{t("timeline.noTimelines")}</p>
                        </div>
                      )}
                    </div>
                  </AccordionItem>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className={`flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
                      ...
                      <h3 className={`text-lg font-semibold flex-1 truncate ${isRTL ? "text-right" : ""}`}>{selectedTimeline.name}</h3>
                      <Button
                        size="sm"
                        color="primary"
                        className="flex-shrink-0"
                        onClick={() => {
                          console.log("Navigating to:", selectedProjectId, selectedTimeline.id);
                          handleProjectClick(selectedProjectId!, selectedTimeline.id);
                        }}
                      >
                        {t("common.viewDetails")}
                      </Button>
                    </div>
                    <p className={`text-sm text-default-500 ${isRTL ? "text-right" : ""}`}>{selectedProjectName}</p>
                    {selectedTimeline.description && (
                      <p className={`text-sm text-default-600 ${isRTL ? "text-right" : ""}`}>{selectedTimeline.description}</p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-lg bg-success/10">
                      <div className={`flex items-center gap-2 mb-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                        ...
                        <p className={`text-xs text-default-500 ${isRTL ? "text-right" : ""}`}>{t("timeline.sprints")}</p>
                      </div>
                      <p className={`text-xl font-semibold text-success block ${isRTL ? "text-right" : ""}`}>
                    ...
                    <div className="p-3 rounded-lg bg-primary/10">
                      <div className={`flex items-center gap-2 mb-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                        ...
                        <p className={`text-xs text-default-500 ${isRTL ? "text-right" : ""}`}>{t("timeline.tasks")}</p>
                      </div>
                      <p className={`text-xl font-semibold text-primary block ${isRTL ? "text-right" : ""}`}>
                  ...
                  <div className="space-y-3">
                    <h4 className={`text-sm font-semibold text-default-700 ${isRTL ? "text-right" : ""}`}>
                      {t("timeline.sprints")}
                    </h4>
                    {selectedTimeline.sprints && selectedTimeline.sprints.length > 0 ? (
                      <div className="space-y-2">
                        {selectedTimeline.sprints.map((sprint) => (
                          <Card key={sprint.id} className="shadow-sm">
                            <CardBody className="p-3">
                              <div className="space-y-2">
                                <p className={`text-sm font-medium ${isRTL ? "text-right" : ""}`}>{sprint.name}</p>
                                {sprint.description && (
                                  <p className={`text-xs text-default-500 line-clamp-2 ${isRTL ? "text-right" : ""}`}>
                                    {sprint.description}
                                  </p>
                                )}
                                <div className={`flex items-center gap-3 text-xs text-default-500 ${isRTL ? "flex-row-reverse justify-end" : ""}`}>
                                  <div className={`flex items-center gap-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                                    <Clock className="w-3 h-3" />
                                    <span>{sprint.tasks?.length || 0} {t("timeline.tasks")}</span>
                                  </div>
                                  <div className={`flex items-center gap-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                                    <Calendar className="w-3 h-3" />
                                    <span>{sprint.duration} {t("common.days")}</span>
                                  </div>
                                </div>
                                ...
                                {sprint.tasks && sprint.tasks.length > 0 && (
                                  <div className="mt-3 pt-2 border-t border-default-200">
                                    <p className={`text-xs font-medium text-default-600 mb-2 block w-full ${isRTL ? "text-right" : ""}`}>
                                      {t("timeline.tasks")}:
                                    </p>
                                    <div className="space-y-1.5">
                                      {sprint.tasks.map((task) => {
                                        // Backend returns AssignedMembers (PascalCase), frontend expects members
                                        const taskMembers = (task as any).assignedMembers || (task as any).AssignedMembers || task.members || [];
                                        
                                        // Build tooltip content
                                        const tooltipContent = (
                                          <div className="p-2 space-y-1.5 max-w-xs" dir={isRTL ? "rtl" : "ltr"}>
                                            <p className={`font-semibold text-sm ${isRTL ? "text-right" : ""}`}>{task.name}</p>
                                            {task.description && (
                                              <p className={`text-xs text-default-600 ${isRTL ? "text-right" : ""}`}>
                                                {task.description}
                                              </p>
                                            )}
                                            ...
                                            {taskMembers && taskMembers.length > 0 && (
                                              <div className="space-y-1">
                                                ...
                                                <div className={`space-y-0.5 ${isRTL ? "pr-4 text-right" : "pl-4"}`}>
                                                  {taskMembers.map((member: any, idx: number) => (
                                        ...
                                        return (
                                          <Tooltip
                                            ...
                                            delay={300}
                                          >
                                            <div className={`flex items-start gap-2 text-xs cursor-help hover:bg-default-100 rounded p-1 transition-colors px-2 ${isRTL ? "flex-row-reverse text-right" : ""}`}>
                                              <div className={`flex-1 min-w-0 ${isRTL ? "text-right" : ""}`}>
                                                ...
                                                {task.progress !== undefined && (
                                                  <div className={`flex items-center gap-2 mt-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                                                    <div className="flex-1 h-1 bg-default-200 rounded-full overflow-hidden" dir={isRTL ? "rtl" : "ltr"}>
                                                      <div 
                      ...
                      <div className="text-center py-6">
                        ...
                        <p className="text-sm text-default-400">{t("timeline.noSprints")}</p>
                      </div>
                    )}
                  </div>
                </div>
