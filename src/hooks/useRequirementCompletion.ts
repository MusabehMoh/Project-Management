...
interface UseRequirementCompletionReturn {
  ...
  changePeriod: (period: "week" | "month" | "quarter" | "year") => Promise<void>;
...
export function useRequirementCompletion(
  ...
  // State
  const [analytics, setAnalytics] = useState<RequirementCompletionAnalytics | null>(null);
  const [metrics, setMetrics] = useState<RequirementCompletionMetrics | null>(null);
  ...
  const [currentPeriod, setCurrentPeriod] = useState<"week" | "month" | "quarter" | "year">(initialPeriod);
  const [currentAnalystId, setCurrentAnalystId] = useState<number | undefined>(analystId);
  ...
  const changePeriod = useCallback(async (period: "week" | "month" | "quarter" | "year") => {
    setCurrentPeriod(period);
    await fetchMetrics(period, currentAnalystId);
  }, [fetchMetrics, currentAnalystId]);

  // Change analyst and fetch new metrics
  const changeAnalyst = useCallback(async (newAnalystId?: number) => {
