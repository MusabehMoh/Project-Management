...
export const useProjects = (initialFilters?: ProjectFilters) => {
  ...
  const loadProjects = useCallback(async (page = 1, limit = pageSize) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updateProject = useCallback(async (projectData: UpdateProjectRequest): Promise<Project | null> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const filterUsers = useCallback((searchValue: string): User[] => {
    // Use the unified search service for consistent search behavior
    const results = SearchService.universalSearch(users as any[], searchValue) as User[];
    // Sort by relevance for better user experience  
    return SearchService.sortByRelevance(results as any[], searchValue) as User[];
  }, [users]);
