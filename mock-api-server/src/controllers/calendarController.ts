...
export class CalendarController {
  ...
  async getCalendarEvents(req: Request, res: Response) {
    try {
      ...
      if (startDate && endDate) {
        filteredEvents = getEventsByDateRange(
          startDate as string,
          endDate as string,
        );
      }
      ...
      if (priorities) {
        const priorityArray = (priorities as string).split(
          ",",
        ) as CalendarEvent["priority"][];

        filteredEvents = filteredEvents.filter((event) =>
          priorityArray.includes(event.priority),
        );
      }

      // Filter by project IDs
      if (projectIds) {
        const projectIdArray = (projectIds as string)
          .split(",")
          .map((id) => parseInt(id));
      ...
      if (assignedTo) {
        const userIdArray = (assignedTo as string)
          .split(",")
          .map((id) => parseInt(id));
        ...
        filteredEvents = filteredEvents.filter(
          (event) =>
            event.assignedTo &&
            event.assignedTo.some((userId) => userIdArray.includes(userId)),
        );
      }
      ...
      logger.info(
        `Retrieved ${paginatedEvents.length} of ${total} calendar events (page ${pageNum})`,
      );
  ...
  async createCalendarEvent(req: Request, res: Response) {
    try {
      ...
      logger.info(
        `Created new calendar event: ${newEvent.title} (ID: ${newId})`,
      );
  ...
  async updateCalendarEvent(req: Request, res: Response) {
    try {
      ...
      logger.info(
        `Updated calendar event: ${mockCalendarEvents[eventIndex].title} (ID: ${eventId})`,
      );
  ...
  async deleteCalendarEvent(req: Request, res: Response) {
    try {
      ...
      logger.info(
        `Deleted calendar event: ${deletedEvent.title} (ID: ${eventId})`,
      );
  ...
  async getEventsByProject(req: Request, res: Response) {
    try {
      ...
      logger.info(
        `Retrieved ${events.length} events for project ID: ${projectId}`,
      );
