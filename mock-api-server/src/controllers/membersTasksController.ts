  ...
  try {
    const params: TaskSearchParams = {
      ...
      memberIds: req.query.memberIds
        ? (req.query.memberIds as string).split(",").map((id) => parseInt(id))
        : undefined,
      ...
      statusIds: req.query.statusIds
        ? (req.query.statusIds as string).split(",").map((id) => parseInt(id))
        : undefined,
      priorityIds: req.query.priorityIds
        ? (req.query.priorityIds as string).split(",").map((id) => parseInt(id))
        : undefined,
...
export const exportMembersTasks = async (req: Request, res: Response) => {
  try {
    ...
    if (format === "csv") {
      // Generate CSV
      const csvHeader =
        "Task Name,Description,Primary Assignee,All Assignees,Department,Status,Priority,Progress,Start Date,End Date,Time Spent,Estimated Time,Overdue,Tags\n";
      const csvData = filtered
        .map((task) => {
          const primaryAssignee = task.primaryAssignee
            ? `${task.primaryAssignee.gradeName} ${task.primaryAssignee.fullName}`
            : "N/A";
          const allAssignees = task.assignedMembers
            .map((m) => `${m.gradeName} ${m.fullName}`)
            .join("; ");
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="members-tasks-${new Date().toISOString().split("T")[0]}.csv"`,
      );
