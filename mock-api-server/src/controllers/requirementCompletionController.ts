...
export class RequirementCompletionController {
  ...
  async getCompletionAnalytics(req: Request, res: Response) {
    ...
    try {
      ...
      const inDevelopmentRequirements = requirements.filter(req => req.status === "in-development");
      ...
      const atRiskRequirements = [...inDevelopmentRequirements, ...approvedRequirements].filter(req => {
        const expectedDate = new Date(req.expectedCompletionDate);
        const daysUntilDeadline = Math.ceil((expectedDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        return daysUntilDeadline <= 7 && daysUntilDeadline > 0;
      });

      // Calculate average completion time
      const avgCompletionTime = lateCompleted.length > 0 
        ? lateCompleted.reduce((sum, req) => {
            const expectedDate = new Date(req.expectedCompletionDate);
            const completedDate = new Date(req.updatedAt);
            const daysLate = Math.ceil((completedDate.getTime() - expectedDate.getTime()) / (1000 * 60 * 60 * 24));
            return sum + daysLate;
      ...
      const byPriority = {
        high: {
          ...
          onTime: onTimeCompleted.filter(req => req.priority === "high").length,
          late: lateCompleted.filter(req => req.priority === "high").length,
          overdue: overdueRequirements.filter(req => req.priority === "high").length,
        },
        medium: {
          total: requirements.filter(req => req.priority === "medium").length,
          onTime: onTimeCompleted.filter(req => req.priority === "medium").length,
          late: lateCompleted.filter(req => req.priority === "medium").length,
          overdue: overdueRequirements.filter(req => req.priority === "medium").length,
        },
        low: {
          total: requirements.filter(req => req.priority === "low").length,
          onTime: onTimeCompleted.filter(req => req.priority === "low").length,
          late: lateCompleted.filter(req => req.priority === "low").length,
          overdue: overdueRequirements.filter(req => req.priority === "low").length,
        },
      };
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        monthlyTrend.push({
          ...
          onTimeRate: monthRequirements.length > 0 
            ? Math.round((monthOnTime.length / monthRequirements.length) * 100)
            : 0,
        });
      }
      ...
      const analytics = {
        summary: {
          ...
          onTimeRate: completedRequirements.length > 0 
            ? Math.round((onTimeCompleted.length / completedRequirements.length) * 100)
            : 0,
        ...
        overdueItems: overdueRequirements.map(req => ({
          ...
          daysOverdue: Math.ceil((now.getTime() - new Date(req.expectedCompletionDate).getTime()) / (1000 * 60 * 60 * 24)),
        ...
        atRiskItems: atRiskRequirements.map(req => ({
          ...
          priority: req.priority,
          expectedDate: req.expectedCompletionDate,
          daysUntilDeadline: Math.ceil((new Date(req.expectedCompletionDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
          assignedAnalyst: req.assignedAnalyst,
          projectName: req.project?.applicationName,
        })),
      };
  ...
  async getCompletionMetrics(req: Request, res: Response) {
    ...
    try {
      ...
      logger.info(`Fetching completion metrics for period: ${period}, analyst: ${analystId}`);

      let requirements = mockProjectRequirements;
      
      // Filter by analyst if specified
      if (analystId) {
        requirements = requirements.filter(req => req.assignedAnalyst === parseInt(analystId as string));
      }
      ...
      const completedInPeriod = periodRequirements.filter(req => req.status === "completed");
      ...
      const metrics = {
        ...
        completionRate: periodRequirements.length > 0 
          ? Math.round((completedInPeriod.length / periodRequirements.length) * 100)
          : 0,
        onTimeRate: completedInPeriod.length > 0 
          ? Math.round((onTimeInPeriod.length / completedInPeriod.length) * 100)
          : 0,
      };
...
export const requirementCompletionController = new RequirementCompletionController();